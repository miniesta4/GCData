...
run_analysis <- function() {
 ...
 ## Read test sets
	...
	x_test <- read.table("./test/X_test.txt", colClasses=classes, nrows=2947, comment.char="")
 ...
 ## Read training sets
	...
	x_train <- read.table("./train/X_train.txt", colClasses=classes, nrows=7352, comment.char="")
 ...
 ## Mean by activity and subject
	...
	write.table (total_cast, file="table_v03.txt", quote=FALSE, row.names=FALSE)
